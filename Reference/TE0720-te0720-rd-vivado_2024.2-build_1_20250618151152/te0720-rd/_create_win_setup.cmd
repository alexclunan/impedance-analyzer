...
@if %XIL_VIV_EXIST%==1 (
  @echo Use Xilinx installation from '%XILDIR%\%VIVADO_VERSION%\Vivado\'
  @GOTO START_CONFIG
)
@if %XIL_VLAB_EXIST%==1 (
@GOTO ASK_LAB
...
echo '%XILDIR%/%VIVADO_VERSION%/Vivado/' did not exists.
@if %CHECK_DIR_DONE%==1 (
 @GOTO CHANGE_VERSION
)
...
@if "%new_lab%"=="n" (
  @echo Use Xilinx installation from '%XILDIR%\%VIVADO_VERSION%\Vivado_Lab\'
...
@set XIL_VIV_EXIST=0
:CHECK_XILINX_DIR
@set VIVADO_XSETTINGS=%XILDIR%\%VIVADO_VERSION%\Vivado\settings64.bat
@set VITIS_XSETTINGS=%XILDIR%\%VIVADO_VERSION%\Vitis\settings64.bat
@set LABTOOL_XSETTINGS=%XILDIR%\%VIVADO_VERSION%\Vivado_Lab\settings64.bat
@if exist %XILDIR%\%VIVADO_VERSION%\Vivado\ (
  @set XIL_VIV_EXIST=1
  ...
  @set XIL_VIT_EXIST=0
  @if exist %XILDIR%\%VIVADO_VERSION%\Vitis\ (
    @set XIL_VIT_EXIST=1
...
@if exist %XILDIR%\%VIVADO_VERSION%\Vivado_Lab\ (
